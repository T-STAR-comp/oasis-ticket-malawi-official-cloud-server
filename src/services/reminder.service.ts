...
const ORGANIZER_MISSING_LINK_WINDOWS_HOURS = [12, 8, 4] as const;
...
function formatMalawiDateTime(value: string | Date) {
...
async function processVirtualAttendeePrestartReminders() {
  ...
  try {
    const [result] = await pool.query<RowDataPacket[]>(
      `SELECT ut.id AS user_ticket_id, ut.reference,
              u.email, u.full_name,
  ...
  for (const row of rows) {
    try {
      ...
      await pool.query(
        `INSERT INTO virtual_join_reminder_log
...
async function processOrganizerMissingSessionLinks() {
  for (const windowHours of ORGANIZER_MISSING_LINK_WINDOWS_HOURS) {
    ...
    try {
      const [result] = await pool.query<RowDataPacket[]>(
        `SELECT l.id AS listing_id, l.title AS listing_title,
    ...
    for (const row of rows) {
      try {
        await emailService.sendOrganizerMissingSessionLinkReminderEmail({
          ...
          startsAtLabel: formatMalawiDateTime(row.starts_at as string),
...
function logReminderError(label: string, err: unknown) {
...
export function startReminderPoller() {
  ...
  void processVirtualAttendeePrestartReminders().catch((err) =>
    logReminderError("virtual prestart tick failed", err),
  );
  void processVirtualOngoingSessionPrestartReminders().catch((err) =>
    logReminderError("ongoing session prestart tick failed", err),
  );
  void processOrganizerMissingSessionLinks().catch((err) =>
    logReminderError("organizer link tick failed", err),
  );
  setInterval(
    () => void processEventReminders().catch((err) => logReminderError("tick failed", err)),
    hourMs,
  );
  setInterval(
    () =>
