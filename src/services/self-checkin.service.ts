...
function makeGateToken(): string {
  return crypto.randomBytes(24).toString("hex");
}

export function buildGateQrPayload(sessionId: string, gateToken: string) {
  return JSON.stringify({ t: "self_checkin", s: sessionId, g: gateToken });
}

export async function activateSelfCheckin(userId: string, listingId: string) {
  const allowed = await canVerifyListing(userId, listingId);
  if (!allowed) throw new Error("You do not have permission to activate self check-in for this event");

  const [existing] = await pool.query<RowDataPacket[]>(
    `SELECT id FROM self_checkin_sessions WHERE listing_id = :listingId AND status = 'active' LIMIT 1`,
    { listingId },
  );
  if (existing[0]) {
  ...
  await pool.query(
    `INSERT INTO self_checkin_sessions (id, listing_id, activated_by_user_id, gate_token, status)
     VALUES (:id, :listingId, :userId, :gateToken, 'active')`,
    { id, listingId, userId, gateToken },
  );
...
export async function endSelfCheckin(userId: string, sessionId: string) {
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT * FROM self_checkin_sessions WHERE id = :id LIMIT 1`,
    { id: sessionId },
  );
  ...
  await pool.query(
    `UPDATE self_checkin_sessions SET status = 'ended', ended_at = NOW() WHERE id = :id`,
    { id: sessionId },
  );
}

export async function getActiveSelfCheckinForListing(listingId: string) {
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT * FROM self_checkin_sessions WHERE listing_id = :listingId AND status = 'active' ORDER BY started_at DESC LIMIT 1`,
    { listingId },
  );
  const s = rows[0];
  if (!s) return null;
...
export async function getSelfCheckinLogs(userId: string, sessionId: string, since?: string) {
...
function parseGatePayload(raw: string): { sessionId: string; gateToken: string } | null {
  try {
    const data = JSON.parse(raw) as { t?: string; s?: string; g?: string };
...
export async function performSelfCheckin(
  holderUserId: string,
  userTicketId: string,
  gatePayloadRaw: string,
) {
  const parsed = parseGatePayload(gatePayloadRaw.trim());
  if (!parsed) throw new Error("Invalid gate QR code");
  ...
  try {
    ...
    const [sessions] = await conn.query<RowDataPacket[]>(
      `SELECT * FROM self_checkin_sessions WHERE id = :id AND status = 'active' FOR UPDATE`,
      { id: parsed.sessionId },
    );
    const session = sessions[0];
    ...
    const [tickets] = await conn.query<RowDataPacket[]>(
      `SELECT ut.*, u.full_name, l.title AS listing_title, l.event_format
       ...
       FOR UPDATE`,
      { ticketId: userTicketId, userId: holderUserId },
    );
    ...
    await conn.query(
      `UPDATE user_tickets SET status = 'used', verified_at = NOW(), verified_by_user_id = :userId
       WHERE id = :id`,
      { id: userTicketId, userId: holderUserId },
    );
...
export async function performSelfCheckinForGuest(
  userTicketId: string,
  guestEmail: string,
  gatePayloadRaw: string,
) {
  ...
  try {
    ...
    const [tickets] = await conn.query<RowDataPacket[]>(
      `SELECT ut.*, o.contact_name, o.contact_email, l.title AS listing_title, l.event_format
       FROM user_tickets ut
       ...
       FOR UPDATE`,
      { ticketId: userTicketId, email: guestEmail.trim().toLowerCase() },
    );
    const ticket = tickets[0];
    if (!ticket) throw new Error("Ticket not found");
    ...
    await conn.query(
      `UPDATE user_tickets SET status = 'used', verified_at = NOW() WHERE id = :id`,
      { id: userTicketId },
    );
...
async function logSelfCheckinEvent(
