...
import { listAvailableBanks, type PayoutDestination } from "./payout.service.js";
...
async function assertEsportsWithdrawable(userId: string, amount: number) {
  const dashboard = await getEsportsDashboard(userId);
  if (amount > dashboard.withdrawable) {
    throw new Error(
      `Only ${dashboard.withdrawable.toLocaleString()} MWK is withdrawable from your E-Sports balance.`,
    );
  }
...
export async function initiateEsportsPayoutVerification(
  userId: string,
  input: {
    amount: number;
    bankUuid: string;
  ...
  await pool.query(
    `UPDATE esports_payout_verifications SET status = 'expired'
     WHERE user_id = :userId AND status = 'pending'`,
    { userId },
  );

  await pool.query(
    `INSERT INTO esports_payout_verifications (
...
export async function confirmEsportsPayoutVerification(
  userId: string,
  ...
  await pool.query(
    `UPDATE esports_payout_verifications SET attempt_count = :attempts WHERE id = :verificationId`,
    { attempts, verificationId },
  );

  if (attempts > MAX_VERIFY_ATTEMPTS) {
    await pool.query(
      `UPDATE esports_payout_verifications SET status = 'failed', failure_reason = :reason WHERE id = :verificationId`,
  ...
  await pool.query(
    `UPDATE esports_payout_verifications SET status = 'completed', payout_id = :payoutId WHERE id = :verificationId`,
    { verificationId, payoutId: result.payoutId },
  );

  await pool.query(
    `INSERT INTO esports_payout_destinations (user_id, bank_uuid, bank_name, account_name, account_number, branch)
...
async function executeEsportsPayout(
  userId: string,
  grossAmount: number,
  ...
  try {
    ...
    const [walletRows] = await conn.query<RowDataPacket[]>(
      `SELECT balance_mwk FROM esports_wallets WHERE user_id = :userId FOR UPDATE`,
      { userId },
    );
    ...
    await conn.query(
      `UPDATE esports_wallets SET balance_mwk = balance_mwk - :amount WHERE user_id = :userId`,
      { userId, amount: grossAmount },
    );

    if (env.paychangu.mock) {
      await conn.query(
        `INSERT INTO esports_payouts (
  ...
  try {
    ...
    await pool.query(
      `UPDATE esports_payouts SET status = 'failed', failure_reason = :reason, provider_status = 'failed'
       WHERE id = :id`,
