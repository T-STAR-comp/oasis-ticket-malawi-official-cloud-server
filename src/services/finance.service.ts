...
import {
  ...
  getOrganizerSettlementBalances,
  getOrganizerSettlementLines,
  getPlatformSettlementBalances,
} from "./settlement.service.js";

function mapTransaction(row: RowDataPacket) {
  const seatsRaw = row.seats as string | null;
  return {
    orderId: row.orderId as string,
    reference: row.reference as string,
    createdAt: row.createdAt as string,
    orderStatus: row.orderStatus as string,
    ledgerStatus: (row.ledgerStatus as string | null) ?? null,
    paymentMethod: row.paymentMethod as string,
    listingId: row.listingId as string,
    listingTitle: row.listingTitle as string,
    organizerName: (row.organizerName as string | null) ?? undefined,
    subtotal: Number(row.subtotal ?? 0),
    serviceFee: Number(row.serviceFee ?? 0),
    total: Number(row.total ?? 0),
    seats: seatsRaw ? seatsRaw.split(",").filter(Boolean) : [],
    failureReason: (row.failureReason as string | null) ?? undefined,
  };
}

export async function getOrganizerFinance(organizerId: string) {
...
export async function getAdminFinance() {
  ...
  const [byOrganizer] = await pool.query<RowDataPacket[]>(
    `SELECT op.user_id AS organizerId, op.company_name AS companyName,
       COALESCE(SUM(CASE WHEN o.status = 'confirmed' THEN o.total_mwk ELSE 0 END), 0) AS revenue,
       COUNT(CASE WHEN o.status = 'confirmed' THEN 1 END) AS orders
