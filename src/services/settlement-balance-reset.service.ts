...
export async function applySettlementEpochResetOnStartup() {
  ...
  const { expired: expiredStaleVerifications, deletedOrphans: deletedOrphanVerifications } =
    await expireStalePayoutVerifications();
...
async function cancelStaleOrganizerPayouts(): Promise<number> {
  const params = { epochDate: SETTLEMENT_EPOCH_DATE };

  const [orphanResult] = await pool.query<ResultSetHeader>(
    `DELETE p FROM organizer_payouts p
     LEFT JOIN users u ON u.id = p.organizer_id
     WHERE u.id IS NULL
       AND p.status IN ('pending', 'processing')
       AND DATE(p.requested_at) < :epochDate`,
    params,
  );

  const [updateResult] = await pool.query<ResultSetHeader>(
    `UPDATE organizer_payouts p
...
async function expireStalePayoutVerifications(): Promise<{
  expired: number;
  ...
  const [orphanResult] = await pool.query<ResultSetHeader>(
    `DELETE pv FROM payout_verifications pv
     LEFT JOIN users u ON u.id = pv.organizer_id
     WHERE u.id IS NULL
       AND pv.status = 'pending'
       AND DATE(pv.created_at) < :epochDate`,
    params,
  );

  const [updateResult] = await pool.query<ResultSetHeader>(
    `UPDATE payout_verifications pv
...
async function writeOffPreEpochRefundDebt(): Promise<number> {
  const [debtRows] = await pool.query<RowDataPacket[]>(
    `SELECT COUNT(*) AS cnt FROM organizer_profiles op
  ...
  if (preEpochDebtWriteoffs <= 0) return 0;
