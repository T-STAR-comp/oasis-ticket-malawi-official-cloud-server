...
export async function applySettlementEpochResetOnStartup() {
  const [pendingRows] = await pool.query<RowDataPacket[]>(
    `SELECT COUNT(*) AS cnt FROM organizer_payouts
     WHERE status IN ('pending', 'processing')
       AND DATE(requested_at) < :epochDate`,
  ...
  const [verificationRows] = await pool.query<RowDataPacket[]>(
    `SELECT COUNT(*) AS cnt FROM payout_verifications
     WHERE status = 'pending'
       AND DATE(created_at) < :epochDate`,
    { epochDate: SETTLEMENT_EPOCH_DATE },
  );
