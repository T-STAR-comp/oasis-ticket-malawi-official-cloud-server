...
function payoutDetailsFromApplication(app: RowDataPacket) {
  const bankName =
    (app.bank_name as string | null) ??
    (app.settlement_preference as string | null) ??
    null;
  const bankUuid = resolveBankUuid(bankName);
  if (!bankUuid || !app.account_name || !app.account_number) return null;
  return {
    bankUuid,
...
async function applyPayoutDetailsToOrganizer(userId: string, app: RowDataPacket) {
  ...
  await pool.query(
    ...
    { userId, ...payout } satisfies QueryParams,
  );
}
...
export async function adminSignIn(username: string, password: string): Promise<AuthUser | null> {
...
export async function changeAdminPassword(adminId: string, currentPassword: string, newPassword: string) {
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT password_hash, email, full_name FROM users WHERE id = :id AND role = 'admin'`,
    { id: adminId },
  );
  const row = rows[0];
...
export async function listUsers() {
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT id, email, username, full_name, phone, role, status, email_verified, created_at
...
export async function updateUserStatus(userId: string, status: "active" | "suspended" | "inactive") {
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT email, full_name, role FROM users WHERE id = :id AND role != 'admin'`,
    { id: userId },
  );
  const user = rows[0];
...
export async function listOrganizers() {
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT u.id AS user_id, u.email, u.full_name, u.status AS user_status, u.created_at,
...
export async function updateOrganizerStatus(
  ...
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT op.company_name, op.email, u.full_name
  ...
  if (status === "suspended" || status === "inactive") {
    ...
    if (status === "suspended") {
      await emailService.sendOrganizerSuspendedBuyerNotice(userId, row.company_name as string);
    }
  ...
  await emailService.sendOrganizerStatusEmail(
    row.email as string,
...
export async function getPartnerApplication(id: string) {
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT * FROM partner_applications WHERE id = :id`,
    { id },
  );
  return rows[0] ?? null;
}
...
export async function reviewPartnerApplication(
  ...
  if (!["submitted", "reviewing"].includes(app.status as string)) {
  ...
  await pool.query(
    `UPDATE partner_applications
     ...
     WHERE id = :id`,
    { status: decision, adminId, notes: adminNotes ?? null, id },
  );
  ...
  if (decision === "approved") {
    ...
    const [existing] = await pool.query<RowDataPacket[]>(
      ...
      { email: (app.contact_email as string).toLowerCase() },
    );

    if (existing[0]) {
      userId = existing[0].id as string;
      await pool.query(
        `UPDATE users SET role = 'organizer', status = 'active', email_verified = 1, email_verified_at = COALESCE(email_verified_at, NOW())
         WHERE id = :userId`,
        { userId },
      );
      ...
      if (!profile[0]) {
        await pool.query(
          `INSERT INTO organizer_profiles (id, user_id, company_name, contact_name, email, phone, partner_type, city, bio, status)
        ...
        await pool.query(
          `UPDATE organizer_profiles SET status = 'approved', company_name = :companyName WHERE user_id = :userId`,
          { userId, companyName: app.company_name },
        );
      }
      await applyPayoutDetailsToOrganizer(userId, app);
    } else {
      ...
      await pool.query(
        `INSERT INTO users (id, email, password_hash, full_name, phone, role, status, email_verified, email_verified_at)
      ...
      await emailService.sendEmail(
        app.contact_email as string,
        "Your Ticket Malawi organizer account",
        `<p>Your partner application was approved.</p>
  ...
  await emailService.sendPartnerDecisionEmail(
    app.contact_email as string,
...
export async function getDashboardStats() {
  ...
  const [organizers] = await pool.query<RowDataPacket[]>(
    `SELECT COUNT(*) AS total FROM organizer_profiles`,
  );
  const [pendingApps] = await pool.query<RowDataPacket[]>(
    `SELECT COUNT(*) AS total FROM partner_applications WHERE status IN ('submitted','reviewing')`,
  );
  const [suspended] = await pool.query<RowDataPacket[]>(
