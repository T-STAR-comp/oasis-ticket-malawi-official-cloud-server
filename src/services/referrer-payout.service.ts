...
async function assertReferrerWithdrawable(userId: string, amount: number) {
  const balances = await getReferrerFinance(userId);
  if (amount > balances.withdrawable) {
    throw new Error(
      `Only ${balances.withdrawable.toLocaleString()} MWK is withdrawable right now. Referral earnings settle on T+1.`,
    );
  }
...
export async function getReferrerPayoutDestination(userId: string): Promise<PayoutDestination | null> {
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT payout_bank_uuid, payout_bank_name, payout_account_name, payout_account_number
     FROM referrer_profiles WHERE user_id = :userId`,
    { userId },
  );
  const row = rows[0];
  if (!row?.payout_bank_uuid) return null;
  return {
...
export async function listReferrerPayouts(userId: string, limit = 50) {
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT id, amount_mwk, platform_fee_mwk, status, paychangu_charge_id,
            bank_account_name, provider_status, failure_reason, requested_at, completed_at
     FROM referrer_payouts
     WHERE referrer_user_id = :userId
     ORDER BY requested_at DESC LIMIT ${Number(limit)}`,
    { userId },
  );
  return rows.map((r) => ({
    id: r.id as string,
...
export async function initiateReferrerPayoutVerification(
  userId: string,
  input: {
    amount: number;
    bankUuid: string;
  ...
  await pool.query(
    `UPDATE referrer_payout_verifications SET status = 'expired'
     WHERE referrer_user_id = :userId AND status = 'pending'`,
    { userId },
  );

  await pool.query(
    `INSERT INTO referrer_payout_verifications (
...
export async function confirmReferrerPayoutVerification(
  userId: string,
  ...
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT *, (expires_at > NOW()) AS not_expired
     ...
     WHERE id = :verificationId AND referrer_user_id = :userId`,
  ...
  await pool.query(
    `UPDATE referrer_payout_verifications SET attempt_count = :attempts WHERE id = :verificationId`,
    { attempts, verificationId },
  );

  if (attempts > MAX_VERIFY_ATTEMPTS) {
    await pool.query(
      `UPDATE referrer_payout_verifications SET status = 'failed', failure_reason = :reason WHERE id = :verificationId`,
  ...
  await pool.query(
    `UPDATE referrer_payout_verifications SET status = 'completed', payout_id = :payoutId WHERE id = :verificationId`,
    { verificationId, payoutId: result.payoutId },
  );

  await pool.query(
    `INSERT INTO referrer_profiles (user_id, payout_bank_uuid, payout_bank_name, payout_account_name, payout_account_number)
...
async function executeReferrerPayout(
  userId: string,
  grossAmount: number,
  ...
  try {
    ...
    await pool.query(
      `UPDATE referrer_payouts SET status = 'failed', failure_reason = :reason, provider_status = 'failed'
       WHERE id = :id`,
  ...
  await pool.query(
    `UPDATE referrer_payouts SET provider_status = :providerStatus WHERE id = :id`,
