...
export async function getTierSoldCount(tierId: string): Promise<number> {
...
async function getTierPendingCount(tierId: string): Promise<number> {
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT COALESCE(SUM(
...
export async function enrichTier(row: TierRow): Promise<TicketTierRow> {
...
export async function listTiersForListing(listingId: string): Promise<TicketTierRow[]> {
  const [rows] = await pool.query<TierRow[]>(
    `SELECT * FROM listing_ticket_tiers WHERE listing_id = :listingId ORDER BY sort_order ASC, created_at ASC`,
    { listingId },
  );
  const tiers: TicketTierRow[] = [];
...
export async function assertTierCheckoutCapacity(
  ...
  const [rows] = await pool.query<TierRow[]>(
    `SELECT * FROM listing_ticket_tiers WHERE id = :tierId LIMIT 1`,
    { tierId },
  );
  const row = rows[0];
  ...
  if (tier.capacity == null) return tier;
  ...
  if (requestedUnits > available) {
    ...
    throw new Error(
      `Only ${available} ${tier.name} ticket${available === 1 ? "" : "s"} remaining.`,
    );
  }
  return tier;
}

export async function saveTiersForListing(
  listingId: string,
  kind: "event" | "travel",
...
export function defaultTierFromListing(price: number): TicketTierRow {
  return {
    id: "default",
    listingId: "",
    name: "Standard",
