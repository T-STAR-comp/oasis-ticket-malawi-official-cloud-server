...
function mapUserRow(row: RowDataPacket): AuthUser {
...
async function upsertFirebaseUser(input: {
  ...
  const [byUid] = await pool.query<RowDataPacket[]>(
    `SELECT id, email, full_name, role, status FROM users WHERE firebase_uid = :firebaseUid`,
    { firebaseUid: input.firebaseUid },
  );
  if (byUid[0]) return mapUserRow(byUid[0]);

  const [byEmail] = await pool.query<RowDataPacket[]>(
    `SELECT id, email, full_name, role, status, firebase_uid FROM users WHERE email = :email`,
    { email },
  );
...
export async function authenticateFirebaseIdToken(
  idToken: string,
  input?: { fullName?: string; phone?: string; acceptedTerms?: boolean },
): Promise<AuthUser> {
  ensureFirebaseAdmin();
