...
export async function getOrganizerRefundDebtSummary(
  organizerId: string,
): Promise<RefundDebtSummary> {
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT refund_debt_mwk, refund_recovered_mwk FROM organizer_profiles WHERE user_id = :organizerId`,
    { organizerId },
  );
  const row = rows[0];
  ...
  const [pendingRows] = await pool.query<RowDataPacket[]>(
    `SELECT COUNT(*) AS cnt FROM ticket_refunds
...
export async function addOrganizerRefundDebt(organizerId: string, amount: number) {
  if (amount <= 0) return;
  await pool.query(
  ...
  try {
    const [profileRows] = await pool.query<RowDataPacket[]>(
      `SELECT op.company_name, op.refund_debt_mwk, op.refund_recovered_mwk, u.email
...
async function fulfillPendingRefunds(organizerId: string, budget: number) {
  ...
  for (const row of pending) {
    ...
    await pool.query(
      `UPDATE orders SET status = 'refunded' WHERE id = :orderId AND status = 'confirmed'`,
 ...
 */
export async function applyRefundRecovery(organizerId: string) {
  ...
  await pool.query(
    `UPDATE organizer_profiles
     ...
     WHERE user_id = :organizerId`,
    { organizerId, amount: toApply },
  );

  await pool.query(
    `INSERT INTO refund_recovery_allocations (id, organizer_id, amount_mwk, source)
     VALUES (:id, :organizerId, :amount, 'settled_sale')`,
    { id: uuid(), organizerId, amount: toApply } satisfies QueryParams,
  );
...
export async function syncOrganizerRefundRecovery(organizerId: string) {
...
export async function syncRefundRecoveryForOrganizersWithDebt() {
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT user_id FROM organizer_profiles WHERE refund_debt_mwk > refund_recovered_mwk`,
  );
