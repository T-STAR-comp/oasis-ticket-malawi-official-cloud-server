...
type SumRow = RowDataPacket & { total: number | null };
type GroupRow = RowDataPacket & { label: string; cnt: number };

async function count(sql: string, params: QueryParams = {}): Promise<number> {
  const [rows] = await pool.query<CountRow[]>(sql, params);
  return Number(rows[0]?.cnt ?? 0);
}

async function sum(sql: string, params: QueryParams = {}): Promise<number> {
  const [rows] = await pool.query<SumRow[]>(sql, params);
...
async function groupCount(
  ...
  const [rows] = await pool.query<GroupRow[]>(sql, params);
...
export async function getAdminStatistics() {
  const [
    ...
    partnerAppsTotal,
    partnerAppsByStatus,
    partnerAppsByPartnerType,
    partnerAppsReviewed,
    partnerAppsPending,
    organizersTotal,
    organizersByStatus,
    organizersByPartnerType,
    organizersWithPayout,
    organizersByCity,
    listingsTotal,
    listingsByKind,
    listingsByStatus,
    listingsPublicVisible,
    listingsWithEventDate,
    listingsUpcoming,
    listingsPastEventDate,
    ...
    seatsByStatus,
    seatLayoutsTotal,
    seatLayoutsByDriverSide,
    ordersTotal,
    ordersByStatus,
    ordersRevenueConfirmed,
    ordersRevenuePending,
    ordersRevenueFailed,
    ...
    userTicketsWithSeat,
    userTicketsRevenue,
    userTicketsUniqueHolders,
    ledgerTotal,
    ledgerByStatus,
    ledgerAmountCompleted,
    ledgerAmountPending,
    ledgerAmountFailed,
    ledgerByPaymentMethod,
    ledgerAvgPollCount,
    verificationsTotal,
    verificationsByResult,
    verificationsByMethod,
    verifierAssignmentsTotal,
    verifierAssignmentsByStatus,
    checkoutQueueTotal,
    checkoutQueueByStatus,
    reminderLogTotal,
    reminderLogByDaysBefore,
    ticketSharesTotal,
    ticketSharesByStatus,
    payoutVerificationsTotal,
    payoutVerificationsByStatus,
    organizerPayoutsTotal,
    organizerPayoutsByStatus,
    organizerPayoutsAmountCompleted,
    organizerPayoutsAmountPending,
    paymentMethodsTotal,
    ...
    emailCodesUsed,
    magicLinksTotal,
    magicLinksByAccountLink,
    magicLinksUsed,
    usersWithUsername,
    usersSuspended,
    partnerAppsByRegion,
    partnerAppsByCity,
    listingsSoldOutWithRemaining,
    listingsTopLocations,
    ordersServiceFeesTotal,
    ordersRefunded,
    orderItemsWithSeats,
    orderItemsAvgQty,
    seatsTaken,
    ...
    userTicketsNew30d,
    userTicketsAvgPaid,
    verificationRejectReasons,
    verifierAssignmentsActive,
    checkoutQueueWaiting,
    ticketSharesUniqueRecipients,
    payoutVerificationsAvgAttempts,
    payoutVerificationsTopBanks,
    organizerPayoutsFailed,
    organizerPayoutsProcessing,
    paymentMethodsUsers,
    ...
    count(
      `SELECT COUNT(*) AS cnt FROM organizer_profiles
       WHERE payout_account_number IS NOT NULL AND payout_account_number != ''`,
    ),
    groupCount(
      `SELECT city AS label, COUNT(*) AS cnt FROM organizer_profiles GROUP BY city ORDER BY cnt DESC LIMIT 15`,
    ),
    ...
    groupCount(
      `SELECT category AS label, COUNT(*) AS cnt FROM listings GROUP BY category ORDER BY cnt DESC LIMIT 20`,
    ),
    ...
    groupCount(
      `SELECT status AS label, COUNT(*) AS cnt FROM listing_verifier_assignments GROUP BY status`,
    ),
    ...
    sum(
      `SELECT COALESCE(SUM(amount_mwk),0) AS total FROM organizer_payouts WHERE status IN ('pending','processing')`,
    ),
    ...
    groupCount(
      `SELECT CASE WHEN user_id IS NULL THEN 'Email only' ELSE 'Linked account' END AS label,
              COUNT(*) AS cnt
    ...
    groupCount(
      `SELECT region AS label, COUNT(*) AS cnt FROM partner_applications GROUP BY region ORDER BY cnt DESC LIMIT 15`,
    ),
    groupCount(
      `SELECT city AS label, COUNT(*) AS cnt FROM partner_applications GROUP BY city ORDER BY cnt DESC LIMIT 15`,
    ),
    count(
    ...
    groupCount(
      `SELECT location AS label, COUNT(*) AS cnt FROM listings GROUP BY location ORDER BY cnt DESC LIMIT 15`,
    ),
    ...
    groupCount(
      `SELECT COALESCE(reject_reason, 'none') AS label, COUNT(*) AS cnt
       FROM ticket_verifications WHERE result = 'rejected'
       GROUP BY reject_reason`,
    ),
    count(
      `SELECT COUNT(*) AS cnt FROM listing_verifier_assignments
    ...
    groupCount(
      `SELECT bank_name AS label, COUNT(*) AS cnt FROM payout_verifications GROUP BY bank_name ORDER BY cnt DESC LIMIT 10`,
    ),
  ...
  const topListingsBySales = await groupCount(
    `SELECT l.title AS label, COUNT(ut.id) AS cnt
  ...
  const topOrganizersByRevenue = await groupCount(
    `SELECT op.company_name AS label, COALESCE(SUM(o.total_mwk),0) AS cnt
     FROM orders o
  ...
  const salesByListingKind = await groupCount(
    `SELECT l.kind AS label, COUNT(ut.id) AS cnt
     FROM user_tickets ut
     JOIN listings l ON l.id = ut.listing_id
     GROUP BY l.kind`,
  );

  const revenueByListingKind = await groupCount(
    `SELECT l.kind AS label, COALESCE(SUM(o.total_mwk), 0) AS cnt
     FROM orders o
     JOIN listings l ON l.id = o.listing_id
     WHERE o.status = 'confirmed'
     GROUP BY l.kind`,
  ...
  const topOrganizersByListings = await groupCount(
    `SELECT op.company_name AS label, COUNT(l.id) AS cnt
     FROM listings l
     JOIN organizer_profiles op ON op.user_id = l.organizer_id
     GROUP BY op.user_id, op.company_name
     ORDER BY cnt DESC LIMIT 10`,
  ...
  const seatOccupancyPercent =
    seatsTotal > 0 ? Math.round((seatsTaken / seatsTotal) * 10000) / 100 : 0;

  const gateVerificationRatePercent =
    userTicketsTotal > 0
