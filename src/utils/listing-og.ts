...
import type { SeoSnapshot } from "./seo-snapshot.js";
...
const listingOgCache = new Map<string, { at: number; snapshot: SeoSnapshot }>();
...
export function resolveListingOgImageUrl(
  ...
  if (url.startsWith("/photos/")) return `${origin}${url}`;
  ...
  if (legacy) return `${origin}${legacy}`;

  if (url.startsWith("/")) return `${origin}${url}`;
  return fallback;
}

export function listingIdForPath(pathname: string): string | null {
...
export async function buildListingSeoSnapshot(
  listingId: string,
  siteOrigin: string,
  ...
  const description = truncate(
    String(raw.description ?? "").trim() || subtitle || `Book ${title} on Ticket Malawi.`,
    300,
  );
...
export function injectListingOgIntoHtml(template: string, snapshot: SeoSnapshot): string {
  let html = template.replace(
    /<!-- tm-listing-og-start -->[\s\S]*?<!-- tm-listing-og-end -->\n?/g,
    "",
  );
