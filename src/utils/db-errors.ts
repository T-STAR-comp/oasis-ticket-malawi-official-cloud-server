...
export function isDuplicateEntryError(err: unknown): err is MySqlError {
  const e = err as MySqlError;
  return e?.code === "ER_DUP_ENTRY" || e?.errno === 1062;
}

export function isReferencedRowError(err: unknown): err is MySqlError {
  const e = err as MySqlError;
...
export function friendlyReferencedMessage(err: unknown): string {
  const sqlMessage = (err as MySqlError)?.sqlMessage?.toLowerCase() ?? "";
