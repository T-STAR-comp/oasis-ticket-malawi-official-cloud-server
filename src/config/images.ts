...
export function resolveImageBucketDir(): string {
  const configured = env.images.bucketDir.trim();
  if (path.isAbsolute(configured)) return configured;
  return path.resolve(path.join(__dirname, "..", ".."), configured);
}

export function isManagedImagePath(imageUrl: string | null | undefined): boolean {
  if (!imageUrl?.trim()) return false;
  const v = imageUrl.trim();
  return (
    v.includes(`${IMAGE_BUCKET_URL_PREFIX}/`) ||
    v.startsWith(`${IMAGE_BUCKET_URL_PREFIX}/`)
  );
}

export function imagePathToAbsoluteFile(imageUrl: string): string | null {
  if (!isManagedImagePath(imageUrl)) return null;
...
export function buildPublicImageUrl(relativePath: string): string {
