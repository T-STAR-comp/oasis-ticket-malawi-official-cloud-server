...
import {
  getEsportsDashboard,
  getEsportsPublicVisibility,
  getEsportsRegistrationStatus,
  getPublicEsportsEvent,
  listPublicEsportsEvents,
  registerForEsportsEvent,
} from "../services/esports.service.js";
import {
  confirmEsportsPayoutVerification,
  initiateEsportsPayoutVerification,
  listAvailableBanks,
} from "../services/esports-payout.service.js";
...
esportsRouter.get("/events/:id", async (req, res, next) => {
  try {
    ...
    if (!event) return fail(res, "Tournament not found", 404);
    return ok(res, event);
  } catch (err) {
    next(err);
  }
...
esportsRouter.post("/events/:id/register", async (req, res, next) => {
  try {
    ...
    if (err instanceof z.ZodError) return fail(res, "Invalid registration request", 400);
...
esportsRouter.get("/registrations/:id/status", async (req, res, next) => {
  try {
    ...
    if (!status) return fail(res, "Registration not found", 404);
...
esportsRouter.post("/payouts/confirm", async (req, res, next) => {
  try {
    ...
    const body = z
      ...
      .parse(req.body);
    const result = await confirmEsportsPayoutVerification(
      user.id,
      body.verificationId,
      body.code,
    );
