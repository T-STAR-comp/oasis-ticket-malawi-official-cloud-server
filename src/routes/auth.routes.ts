...
authRouter.post("/signin", async (req, res, next) => {
  try {
    ...
    if (err instanceof Error) {
      if (err.message === "Email not verified") return fail(res, err.message, 403);
      if (err.message === "Account suspended") return fail(res, err.message, 403);
      if (err.message === "Account inactive") return fail(res, err.message, 403);
    }
    if (err instanceof Error && err.message.includes("Data truncated")) {
      return fail(
        res,
        "Login security codes are not enabled on the database yet. Run: npm run db:migrate:tiers",
        503,
      );
    }
    next(err);
...
authRouter.post("/signin/verify", async (req, res, next) => {
  try {
    ...
    if (!user) return fail(res, "Invalid email or security code", 401);
    const token = signToken(user);
    return ok(res, { user, token });
  } catch (err) {
    if (err instanceof Error) {
      if (err.message.includes("security code")) return fail(res, err.message, 400);
      if (err.message === "Account suspended") return fail(res, err.message, 403);
      if (err.message === "Account inactive") return fail(res, err.message, 403);
    }
    next(err);
...
authRouter.post("/change-password/initiate", requireAuth, async (req, res, next) => {
  try {
    ...
    const body = z
      ...
      .parse(req.body);
    const result = await authService.initiatePasswordChange(
      user.id,
