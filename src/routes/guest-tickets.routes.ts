...
guestTicketsRouter.post("/lookup", async (req, res, next) => {
  try {
    ...
    if (err instanceof z.ZodError) return fail(res, "Invalid lookup request", 400);
...
guestTicketsRouter.post("/pdf", async (req, res, next) => {
  try {
    const body = z
      ...
      .parse(req.body);
    const { buffer, filename } = await guestTicketsService.guestTicketPdf(
      body.reference,
      body.email,
    );
    ...
    if (err instanceof z.ZodError) return fail(res, "Invalid request", 400);
    if (err instanceof Error) return fail(res, err.message, 404);
    next(err);
  }
});
...
guestTicketsRouter.post("/self-checkin", async (req, res, next) => {
  try {
    ...
    const result = await selfCheckinService.performSelfCheckinForGuest(
      ticket.id,
      body.email,
      body.gatePayload,
    );
