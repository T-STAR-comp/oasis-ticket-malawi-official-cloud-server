...
resellRouter.get("/:id", async (req, res, next) => {
  try {
    ...
    if (!listing) return fail(res, "Resale listing not found", 404);
...
resellRouter.post("/:id/checkout", requireAuth, async (req, res, next) => {
  try {
    ...
    const body = z
      ...
      .parse(req.body);
    const result = await resellService.initiateResellCheckout(
      user.id,
    ...
    if (err instanceof ZodError) return fail(res, formatZodError(err), 400);
