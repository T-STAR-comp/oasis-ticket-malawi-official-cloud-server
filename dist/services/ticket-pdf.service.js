...
import { buildPublicImageUrl, imagePathToAbsoluteFile, IMAGE_BUCKET_URL_PREFIX, } from "../config/images.js";
import { env } from "../config/env.js";
import { pool } from "../db/pool.js";
import { getListingById } from "./listings.service.js";
import { BRAND_RGB, drawContentCard, drawOperatorPanel, drawPatternAccentBand, drawPatternHeader, drawPatternPageBackground, getPatternTiles, mm, } from "../utils/ticket-pdf-branding.js";
...
function formatPurchasedOn(value) {
    if (value instanceof Date)
        return value.toISOString();
    return String(value);
}
function resolveListingImageUrl(imageUrl) {
    if (!imageUrl?.trim())
        return null;
    ...
    if (LEGACY_IMAGE_MAP[url])
        return `${apiOrigin()}${LEGACY_IMAGE_MAP[url]}`;
...
async function readImageBytes(imageUrl) {
    ...
    if (resolved.startsWith("/")) {
        const publicRoot = path.join(path.dirname(fileURLToPath(import.meta.url)), "..", "..", "public", "client");
        try {
...
async function loadCoverBannerBuffer(imageUrl, bannerWidthPt, bannerHeightPt) {
    if (!imageUrl?.trim())
        return null;
...
export async function generateTicketPdfBuffer(ctx) {
    const pageW = mm(210);
    const pageH = mm(297);
    const margin = mm(16);
    ...
    return new Promise((resolve, reject) => {
        ...
        const fields = [
            ...
            ...(ctx.ticketTierName ? [["Ticket type", ctx.ticketTierName]] : []),
        ...
        doc.text("This is your official Ticket Malawi e-ticket. Keep it safe and present the QR code at entry.", margin, pageH - mm(8), { width: contentW, align: "center" });
        doc.end();
    });
}
export function buildTicketPdfContext(ticketRow, listing) {
    return {
...
export async function loadTicketsForOrderEmail(orderId) {
    const [rows] = await pool.query(`SELECT ut.id, ut.reference, ut.qr_token, ut.seat_number, ut.amount_paid, ut.ticket_tier_name,
     ...
     ORDER BY ut.purchased_at ASC`, { orderId });
    return rows;
}
export async function buildTicketPdfAttachments(orderId) {
...
export function normalizeGuestKey(key) {
