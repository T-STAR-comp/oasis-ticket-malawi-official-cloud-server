...
import { PAYMENT_COMPLETED_AT, PAYOUT_EPOCH_WHERE, SETTLEMENT_EPOCH_DATE, WITHDRAWABLE_EPOCH_WHERE, } from "../utils/settlement-epoch.js";
...
export async function getOrganizerSettlementBalances(organizerId) {
    ...
    const [rows] = await pool.query(`SELECT
       COALESCE(SUM(
         CASE WHEN ${WITHDRAWABLE_EPOCH_WHERE} THEN o.subtotal_mwk ELSE 0 END
       ), 0) AS totalEarnings,
       COALESCE(SUM(
         CASE
...
export async function getOrganizerSettlementLines(organizerId, limit = 100) {
    const [rows] = await pool.query(`SELECT
       ...
       CASE
         WHEN DATE(${PAYMENT_COMPLETED_AT}) < '${SETTLEMENT_EPOCH_DATE}' THEN 'pre_epoch_excluded'
...
export async function getAdminSettlementByOrganizer() {
    ...
    const payoutRows = await pool.query(`SELECT organizer_id,
    ...
    const payoutMap = new Map(payoutRows[0].map((r) => [
        r.organizer_id,
