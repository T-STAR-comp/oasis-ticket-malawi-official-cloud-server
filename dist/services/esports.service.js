...
import { dgpaApplies, dgpaContributionFromBuyIn, } from "../utils/esports-dgpa.js";
function normalizeEsportsEntry(input) {
...
function esportsSlotsMessage(participantCount, maxSlots, isCompleted) {
    if (isCompleted) {
...
function toPublicEsportsEvent(event) {
    const maxSlots = event.maxSlots;
    const { isSoldOut, slotsMessage } = esportsSlotsMessage(event.participantCount, maxSlots, event.isCompleted);
    return {
        id: event.id,
        name: event.name,
...
function mapEvent(row, participantCount) {
...
export async function getEsportsPublicVisibility() {
    const [rows] = await pool.query(`SELECT COUNT(*) AS cnt FROM esports_events e WHERE ${publicEventFilterSql("e")}`);
...
export async function getPublicEsportsEvent(eventId) {
    const [rows] = await pool.query(`SELECT e.*,
       (SELECT COUNT(*) FROM esports_registrations r
        WHERE r.event_id = e.id AND r.payment_status = 'completed') AS participant_count
     FROM esports_events e
     WHERE e.id = :eventId AND ${publicEventFilterSql("e")}`, { eventId });
    const row = rows[0];
    if (!row)
        return null;
...
export async function getAdminEsportsEvent(eventId) {
    const [rows] = await pool.query(`SELECT e.*,
       (SELECT COUNT(*) FROM esports_registrations r
        WHERE r.event_id = e.id AND r.payment_status = 'completed') AS participant_count
     FROM esports_events e WHERE e.id = :eventId`, { eventId });
    const row = rows[0];
    if (!row)
        return null;
    const [participants] = await pool.query(`SELECT r.id, r.user_id, r.game_username, r.amount_paid_mwk, r.payment_status,
...
export async function createEsportsEvent(adminUserId, input) {
    const id = uuid();
    const startingPrize = Math.max(0, Math.round(input.grandPrizeMwk));
    const entry = normalizeEsportsEntry(input);
    await pool.query(`INSERT INTO esports_events (
...
export async function updateEsportsEvent(eventId, input) {
    const existing = await getAdminEsportsEvent(eventId);
    if (!existing)
        throw new Error("E-Sports event not found");
    if (existing.status === "completed" || existing.status === "archived") {
        throw new Error("Completed events cannot be edited");
    }
    const nextStartingPrize = input.grandPrizeMwk != null ? Math.max(0, Math.round(input.grandPrizeMwk)) : null;
    const syncLivePrize = nextStartingPrize != null && existing.participantCount === 0 ? nextStartingPrize : null;
    const entry = input.isFreeEntry !== undefined || input.entryPriceMwk != null
        ...
        : null;
    await pool.query(`UPDATE esports_events SET
     ...
     WHERE id = :eventId`, {
        ...
        matchDurationMinutes: input.matchDurationMinutes != null
...
export async function updateEsportsMatchAccess(eventId, input) {
    const existing = await getAdminEsportsEvent(eventId);
    if (!existing)
        throw new Error("E-Sports event not found");
    ...
    await pool.query(`UPDATE esports_events SET
     ...
     WHERE id = :eventId`, {
        ...
        matchPassword: input.matchPassword !== undefined ? input.matchPassword?.trim() || null : null,
    });
    return getAdminEsportsEvent(eventId);
}
export async function settleEsportsWinner(eventId, input) {
    ...
    if (!winner)
        throw new Error("Winner must be a registered participant");
    const conn = await pool.getConnection();
    try {
        await conn.beginTransaction();
        await conn.query(`UPDATE esports_events SET
         status = 'completed',
       ...
       WHERE id = :eventId`, {
        ...
        await conn.query(`INSERT INTO esports_wallets (user_id, balance_mwk)
       VALUES (:userId, :prize)
       ON DUPLICATE KEY UPDATE balance_mwk = balance_mwk + :prize`, { userId: winner.userId, prize: event.grandPrizeMwk });
        await conn.commit();
    }
    catch (err) {
        await conn.rollback();
...
export async function archiveExpiredEsportsEvents() {
    const [result] = await pool.query(`UPDATE esports_events SET status = 'archived'
...
async function applyDgpaAfterRegistration(eventId, entryPriceMwk) {
    const conn = await pool.getConnection();
    ...
    try {
        await conn.beginTransaction();
        const [eventRows] = await conn.query(`SELECT * FROM esports_events WHERE id = :eventId FOR UPDATE`, { eventId });
        const eventRow = eventRows[0];
        if (!eventRow) {
            await conn.rollback();
            return;
        }
        const [countRows] = await conn.query(`SELECT COUNT(*) AS cnt FROM esports_registrations
       WHERE event_id = :eventId AND payment_status = 'completed'`, { eventId });
        ...
        if (dgpaApplies(completedCount, maxSlots)) {
            ...
            if (increaseMwk > 0) {
                ...
                await conn.query(`UPDATE esports_events SET grand_prize_mwk = :newPrize WHERE id = :eventId`, { eventId, newPrize });
                prizeIncreased = true;
            }
        }
        await conn.commit();
...
async function ensureWallet(userId) {
    await pool.query(`INSERT IGNORE INTO esports_wallets (user_id, balance_mwk) VALUES (:userId, 0)`, { userId });
}
export async function getEsportsDashboard(userId) {
    await ensureWallet(userId);
    const [walletRows] = await pool.query(`SELECT balance_mwk FROM esports_wallets WHERE user_id = :userId`, { userId });
    const balance = Number(walletRows[0]?.balance_mwk ?? 0);
    const [regRows] = await pool.query(`SELECT r.id AS registrationId, r.game_username AS gameUsername, r.amount_paid_mwk AS amountPaidMwk,
     ...
     ORDER BY e.event_date DESC, e.event_time DESC`, { userId });
    const [payoutRows] = await pool.query(`SELECT COALESCE(SUM(CASE WHEN status IN ('pending','processing') THEN amount_mwk ELSE 0 END), 0) AS reserved,
            COALESCE(SUM(CASE WHEN status = 'completed' THEN amount_mwk ELSE 0 END), 0) AS paidOut
     FROM esports_payouts WHERE user_id = :userId`, { userId });
...
export async function registerForEsportsEvent(userId, eventId, input) {
    if (!features.payments)
        throw new Error("Payments are temporarily unavailable");
    const gameUsername = input.gameUsername.trim();
    if (gameUsername.length < 2)
        throw new Error("Enter your in-game username");
    const [eventRows] = await pool.query(`SELECT e.*,
       (SELECT COUNT(*) FROM esports_registrations r
        WHERE r.event_id = e.id AND r.payment_status = 'completed') AS participant_count
     FROM esports_events e
     WHERE e.id = :eventId AND ${publicEventFilterSql("e")}`, { eventId });
    ...
    const [existing] = await pool.query(`SELECT id, payment_status FROM esports_registrations
     WHERE event_id = :eventId AND user_id = :userId`, { eventId, userId });
    ...
    const isFreeEntry = Boolean(Number(eventRow.is_free_entry ?? 0)) || entryPrice <= 0;
    const registrationId = uuid();
    if (isFreeEntry) {
        await pool.query(`INSERT INTO esports_registrations (
      ...
      )`, { id: registrationId, eventId, userId, gameUsername });
...
export async function getEsportsRegistrationStatus(userId, registrationId) {
    const [rows] = await pool.query(`SELECT r.*, e.name AS eventName FROM esports_registrations r
     JOIN esports_events e ON e.id = r.event_id
     WHERE r.id = :registrationId AND r.user_id = :userId`, { registrationId, userId });
    const row = rows[0];
    if (!row)
        return null;
    return {
...
export async function processPendingEsportsRegistration(registrationId) {
    const [rows] = await pool.query(`SELECT * FROM esports_registrations WHERE id = :registrationId AND payment_status = 'pending'`, { registrationId });
    const row = rows[0];
    if (!row?.paychangu_charge_id)
        return null;
    ...
    if (verify.failed || TERMINAL_PAYMENT_STATUSES.has(verify.providerStatus.toLowerCase())) {
        await pool.query(`UPDATE esports_registrations SET payment_status = 'failed', failure_reason = :reason WHERE id = :id`, { id: registrationId, reason: verify.message || "Payment failed" });
        return { status: "failed", message: verify.message };
    }
    if (verify.success) {
        const amountPaid = Number(row.amount_paid_mwk ?? 0);
        const eventId = String(row.event_id);
        await pool.query(`UPDATE esports_registrations SET payment_status = 'completed', registered_at = NOW(), failure_reason = NULL
...
export async function listPendingEsportsRegistrations() {
    const [rows] = await pool.query(`SELECT id FROM esports_registrations
...
export async function failStaleEsportsRegistrations() {
    ...
    const [result] = await pool.query(`UPDATE esports_registrations SET payment_status = 'failed',
       failure_reason = 'Payment timed out'
     WHERE payment_status = 'pending'
       AND created_at < DATE_SUB(NOW(), INTERVAL :timeoutSec SECOND)`, { timeoutSec: Math.ceil(timeoutMs / 1000) });
    return result.affectedRows;
}
export function resolveEsportsImageUrl(pathOrUrl) {
    if (!pathOrUrl)
        return null;
