...
const MUTED = "#646464";
const INK = "#282828";
const AUDIT_FOOTER = "Oasis Technology and Capital Finance Audit Office";
...
function drawHorizontalBarChart(doc, x, y, width, title, items, valueFormatter = (n) => String(n)) {
    if (items.length === 0) {
...
export async function generateEventAuditPdf(snapshot) {
    return new Promise((resolve, reject) => {
        ...
        doc.moveDown(1);
        let chartY = doc.y;
        chartY = drawHorizontalBarChart(doc, 48, chartY, pageWidth, "Ticket sales by day", snapshot.charts.salesByDay.map((d) => ({
            label: d.day,
            value: d.revenue,
        })), formatMwk);
        ...
        chartY = drawHorizontalBarChart(doc, 48, chartY, pageWidth, "Sales by ticket type", snapshot.charts.salesByTier.map((t) => ({
            label: t.tier,
            value: t.revenue,
        })), formatMwk);
        if (chartY > doc.page.height - 120) {
        ...
        chartY = drawHorizontalBarChart(doc, 48, chartY, pageWidth, "Payment methods", snapshot.charts.paymentMethods.map((p) => ({
            label: p.method.toUpperCase(),
            value: p.amount,
        })), formatMwk);
        chartY = drawHorizontalBarChart(doc, 48, chartY + 8, pageWidth, "Ticket status breakdown", snapshot.charts.ticketStatus.map((s) => ({
            label: s.status,
            value: s.count,
        })));
        doc.addPage();
        ...
        if (snapshot.refunds.length > 0) {
            ...
            for (const refund of snapshot.refunds) {
                doc
                    .fontSize(9)
                    .fillColor(INK)
                    .text(`${refund.reference} · ${formatMwk(refund.amount)} · ${refund.status} · ${formatDateTime(refund.createdAt)}`);
            }
        }
