...
async function assertResellerWithdrawable(userId, amount) {
    const balances = await getSellerResellFinance(userId);
    if (amount > balances.withdrawable) {
        throw new Error(`Only ${balances.withdrawable.toLocaleString()} MWK is withdrawable right now. Resale earnings settle on T+1.`);
    }
...
export async function getResellerPayoutDestination(userId) {
    const [rows] = await pool.query(`SELECT bank_uuid, bank_name, account_name, account_number
     FROM reseller_payout_destinations WHERE user_id = :userId`, { userId });
    const row = rows[0];
    if (!row?.bank_uuid)
        return null;
    return {
...
export async function initiateResellerPayoutVerification(userId, input) {
    ...
    await pool.query(`UPDATE reseller_payout_verifications SET status = 'expired'
     WHERE user_id = :userId AND status = 'pending'`, { userId });
    await pool.query(`INSERT INTO reseller_payout_verifications (
...
export async function confirmResellerPayoutVerification(userId, verificationId, code) {
    ...
    await pool.query(`UPDATE reseller_payout_verifications SET attempt_count = :attempts WHERE id = :verificationId`, { attempts, verificationId });
    if (attempts > MAX_VERIFY_ATTEMPTS) {
        await pool.query(`UPDATE reseller_payout_verifications SET status = 'failed', failure_reason = :reason WHERE id = :verificationId`, { verificationId, reason: "Too many incorrect verification attempts" });
    ...
    await pool.query(`UPDATE reseller_payout_verifications SET status = 'completed', payout_id = :payoutId WHERE id = :verificationId`, { verificationId, payoutId: result.payoutId });
    await pool.query(`INSERT INTO reseller_payout_destinations (user_id, bank_uuid, bank_name, account_name, account_number)
...
async function executeResellerPayout(userId, grossAmount, platformFeeMwk, netToBank, destination) {
    const payoutId = uuid();
    const chargeId = makePayoutChargeId(payoutId);
    if (env.paychangu.mock) {
        await pool.query(`INSERT INTO reseller_payouts (
    ...
    try {
        ...
        await pool.query(`UPDATE reseller_payouts SET status = 'failed', failure_reason = :reason, provider_status = 'failed'
    ...
    await pool.query(`UPDATE reseller_payouts SET provider_status = :providerStatus WHERE id = :id`, { id: payoutId, providerStatus });
    return {
        payoutId,
        chargeId,
        amount: grossAmount,
