...
export async function getSoldCount(listingId, kind) {
    if (kind === "travel") {
        const [rows] = await pool.query(`SELECT COUNT(*) AS cnt FROM seats s
    ...
    const [rows] = await pool.query(`SELECT COALESCE(SUM(oi.quantity), 0) AS cnt
     ...
     WHERE o.listing_id = :listingId AND o.status = 'confirmed'`, { listingId });
    return Number(rows[0]?.cnt ?? 0);
}
export async function getPendingCheckoutUnits(listingId) {
    const [rows] = await pool.query(`SELECT COALESCE(SUM(
     ...
     WHERE o.listing_id = :listingId AND pl.status = 'pending' AND o.status = 'pending'`, { listingId });
    return Number(rows[0]?.cnt ?? 0);
}
export async function resolveTicketCapacity(listingId, kind, ticketCapacity) {
    if (kind === "travel") {
        const [rows] = await pool.query(`SELECT total_seats FROM seat_layouts WHERE listing_id = :listingId LIMIT 1`, { listingId });
...
export async function getCapacityInfo(listingId, kind, ticketCapacity) {
...
export async function syncListingSoldOutStatus(listingId, kind, ticketCapacity) {
    const { ticketCapacity: capacity, soldCount } = await getCapacityInfo(listingId, kind, ticketCapacity);
    if (capacity == null)
        return;
...
export async function assertCheckoutCapacity(listingId, kind, ticketCapacity, requestedUnits) {
    const { ticketCapacity: capacity, soldCount, pendingCount } = await getCapacityInfo(listingId, kind, ticketCapacity);
    if (capacity == null)
        return;
