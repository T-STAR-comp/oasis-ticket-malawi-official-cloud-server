...
export async function setServiceFeeBearer(bearer, adminId) {
    await setSetting("service_fee_bearer", bearer, adminId);
}
export async function isDynamicServiceFeeEnabled() {
    const value = await getSetting("dynamic_service_fee_enabled");
    return value === "true" || value === "1";
}
export async function setDynamicServiceFeeEnabled(enabled, adminId) {
...
export async function replaceDynamicServiceFeeRanges(ranges, adminId) {
    ...
    try {
        ...
        for (const range of normalized) {
            await conn.query(`INSERT INTO dynamic_service_fee_ranges (id, min_mwk, max_mwk, fee_percent, sort_order)
...
export async function deleteDynamicServiceFeeRange(rangeId) {
    await pool.query(`DELETE FROM dynamic_service_fee_ranges WHERE id = :id`, { id: rangeId });
}
export async function getOrganizerCustomServiceFee(organizerUserId) {
    const [rows] = await pool.query(`SELECT fee_percent FROM organizer_custom_service_fees WHERE organizer_user_id = :organizerId`, { organizerId: organizerUserId });
...
export async function lookupOrganizerByEmail(email) {
    ...
    const [rows] = await pool.query(`SELECT u.id, u.email, u.full_name, op.company_name, op.status AS organizer_status,
...
export async function setOrganizerCustomServiceFee(organizerUserId, feePercent, adminId, notes) {
    if (!Number.isFinite(feePercent) || feePercent < 0 || feePercent > 100) {
        throw new Error("Fee percent must be between 0 and 100");
    }
    const [rows] = await pool.query(`SELECT user_id FROM organizer_profiles WHERE user_id = :organizerId`, { organizerId: organizerUserId });
    if (!rows[0])
        throw new Error("Organizer not found");
    await pool.query(`INSERT INTO organizer_custom_service_fees (
...
export async function removeOrganizerCustomServiceFee(organizerUserId) {
    const [result] = await pool.query(`DELETE FROM organizer_custom_service_fees WHERE organizer_user_id = :organizerId`, { organizerId: organizerUserId });
    if (result.affectedRows === 0) {
        throw new Error("Custom fee not found for this organizer");
    }
}
export async function resolveServiceFeePercent(organizerUserId, catalogSubtotalMwk) {
    if (organizerUserId) {
