...
import { buildTicketPdfAttachments, buildTicketPdfContext, generateTicketPdfBuffer, } from "./ticket-pdf.service.js";
import { sendDelayedTicketApologyEmail, sendTicketPurchaseEmail, } from "./email.service.js";
...
export async function emailTicketsForOrder(orderId, options) {
...
export async function lookupGuestTicket(reference, email) {
    const ref = reference.trim();
    const normalizedEmail = email.trim().toLowerCase();
    if (!ref || !normalizedEmail)
        throw new Error("Ticket reference and email are required");
    const [rows] = await pool.query(`SELECT ut.*, o.contact_email, o.contact_name, o.is_guest,
     ...
     WHERE ut.reference = :reference
       ...
       AND o.is_guest = 1
     LIMIT 1`, { reference: ref, email: normalizedEmail });
    const row = rows[0];
    if (!row)
        throw new Error("Ticket not found. Check your reference and email.");
    return {
        id: row.id,
        ...
        tierName: row.ticket_tier_name ?? null,
        ...
        listing: {
            ...
            eventDate: row.date_label ?? null,
            eventTime: row.time_label ?? null,
            location: row.location ?? null,
            eventFormat: row.event_format ?? "physical",
            virtualMeetingUrl: row.virtual_meeting_url ?? null,
            coverImageUrl: row.image_url ?? null,
        },
    };
}
export async function guestTicketPdf(reference, email) {
    const ticket = await lookupGuestTicket(reference, email);
    const listing = await getListingById(ticket.listing.id, true);
    if (!listing)
        throw new Error("Listing not found for this ticket.");
