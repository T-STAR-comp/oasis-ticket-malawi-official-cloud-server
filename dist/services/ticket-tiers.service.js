...
export async function assertTierFulfillmentCapacity(conn, tierId, requestedUnits, excludeOrderId) {
    const [rows] = await conn.query(`SELECT * FROM listing_ticket_tiers WHERE id = :tierId LIMIT 1 FOR UPDATE`, { tierId });
    const row = rows[0];
    if (!row)
    ...
    const [pendingRows] = await conn.query(`SELECT COALESCE(SUM(
     ...
     WHERE pl.status = 'pending' AND o.status = 'pending' AND o.id != :excludeOrderId
       AND JSON_UNQUOTE(JSON_EXTRACT(pl.checkout_meta, '$.tierId')) = :tierId`, { tierId, excludeOrderId });
    ...
    if (requestedUnits > available) {
        ...
        throw new Error(`Only ${available} ${row.name} ticket${available === 1 ? "" : "s"} remaining.`);
    }
}
