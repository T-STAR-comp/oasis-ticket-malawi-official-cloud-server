...
export function formatPayChanguError(body, fallback) {
...
function hasCompletionEvidence(data, transaction) {
    const completedAt = transaction.completed_at ?? data.completed_at;
    if (completedAt)
        return true;
    const auth = (transaction.authorization ?? data.authorization);
    return !!auth?.completed_at;
}
function isTerminalFailure(paymentStatus, topStatus) {
    return (TERMINAL_PAYMENT_STATUSES.has(paymentStatus) || TERMINAL_PAYMENT_STATUSES.has(topStatus));
}
/**
 ...
 */
function interpretVerifyBody(body, httpOk) {
    ...
    const paymentCompleted = (paymentStatus === "success" || paymentStatus === "successful") &&
        hasCompletionEvidence(data, transaction);
    const verifyEndpointCompleted = topStatus === "successful" &&
    ...
    const explicitlyPending = PENDING_STATUSES.has(paymentStatus) ||
...
export async function verifyMobileMoneyCharge(chargeId, initiatedAt) {
    if (env.paychangu.mock) {
    ...
    const res = await fetch(`${env.paychangu.baseUrl}/mobile-money/payments/${encodeURIComponent(chargeId)}/verify`, { method: "GET", headers: authHeaders() });
    const body = await parseJsonResponse(res);
...
export async function refundCardCharge(chargeId) {
    ...
    const topStatus = String(body.status ?? "").toLowerCase();
    if (!res.ok || topStatus === "failed") {
        throw new PayChanguError(formatPayChanguError(body, "Card refund failed"), res.status, body);
    }
    return body;
}
...
export async function initiateMobileMoneyRefund(input) {
    ...
    const topStatus = String(body.status ?? "").toLowerCase();
    if (!res.ok || topStatus === "failed") {
        throw new PayChanguError(formatPayChanguError(body, "Mobile money refund failed"), res.status, body);
    }
    const data = (body.data ?? {});
