...
async function getPayoutById(payoutId) {
    const [rows] = await pool.query(`SELECT id, paychangu_charge_id, status, requested_at
     FROM organizer_payouts WHERE id = :payoutId LIMIT 1`, { payoutId });
    return rows[0] ?? null;
}
export async function listProcessingOrganizerPayouts(limit = 50) {
    const [rows] = await pool.query(`SELECT p.id, p.paychangu_charge_id, p.status, p.requested_at
...
export async function cleanupOrphanOrganizerPayouts() {
    const [result] = await pool.query(`DELETE p FROM organizer_payouts p
...
export async function attemptPayoutReconciliation(payoutId) {
    ...
    const verify = await verifyDirectChargeTransaction(payout.paychangu_charge_id, new Date(payout.requested_at));
    if (verify.success) {
        await pool.query(`UPDATE organizer_payouts
       SET status = 'completed', provider_status = :providerStatus, completed_at = NOW()
       WHERE id = :id`, { id: payoutId, providerStatus: verify.providerStatus });
    ...
    if (verify.failed) {
       ...
       WHERE id = :id`, { id: payoutId, providerStatus: verify.providerStatus, reason });
    ...
    await pool.query(`UPDATE organizer_payouts SET provider_status = :providerStatus WHERE id = :id`, { id: payoutId, providerStatus: verify.providerStatus });
    ...
    for (const payout of payouts) {
        try {
            ...
            if (result.status === "completed")
                ...
                results.failed += 1;
            else if (result.status === "still_processing")
