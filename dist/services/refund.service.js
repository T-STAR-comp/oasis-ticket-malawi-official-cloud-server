...
async function fetchRefundableTickets(filters) {
    ...
    const params = { organizerId: filters.organizerId };
...
async function processRefunds(tickets, organizerId, context) {
    let completed = 0;
    let skipped = 0;
    let totalRefunded = 0;
    ...
    for (const ticket of tickets) {
        ...
        const feeShare = orderTotal > 0 ? Math.round((amountPaid / orderTotal) * orderFee) : 0;
        ...
        if (!settled) {
            await pool.query(`INSERT INTO ticket_refunds (
        ...
        await emailService.sendTicketRefundEmail(ticket.email, ticket.full_name, String(ticket.reference), refundAmount, amountPaid, context, refundPaymentMethodLabel(String(ticket.payment_method ?? "")));
        completed++;
...
export async function processBanRefunds(organizerId) {
...
export async function processListingCancellationRefunds(listingId, organizerId, cancelledListingFundsHeld = 0) {
    ...
    if (coveredByHold > 0) {
       ...
       WHERE user_id = :organizerId`, { organizerId, amount: coveredByHold });
        await pool.query(`INSERT INTO refund_recovery_allocations (id, organizer_id, amount_mwk, source)
       VALUES (:id, :organizerId, :amount, 'cancelled_hold')`, { id: uuid(), organizerId, amount: coveredByHold });
    }
    if (debtIncrease > 0) {
