...
export async function ensureDefaultAdmin() {
    const [rows] = await pool.query(`SELECT id FROM users WHERE username = :username OR (role = 'admin' AND email = :email) LIMIT 1`, { username: env.admin.username, email: env.admin.email });
    if (rows[0]) {
    ...
    await pool.query(`INSERT INTO users (id, email, username, password_hash, full_name, role, status, email_verified, email_verified_at)
