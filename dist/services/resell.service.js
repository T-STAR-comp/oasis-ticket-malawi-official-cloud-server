...
export async function fulfillResellSale(ledger, conn) {
    ...
    const [rlRows] = await conn.query(`SELECT * FROM resell_listings WHERE id = :id AND status = 'active' FOR UPDATE`, { id: resellListingId });
    if (!rlRows[0])
    ...
    await conn.query(`UPDATE resell_listings SET status = 'sold', sold_at = NOW() WHERE id = :id`, { id: resellListingId });
    const saleId = uuid();
