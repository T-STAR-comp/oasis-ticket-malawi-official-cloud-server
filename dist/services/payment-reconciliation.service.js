...
export async function countFulfillmentTickets(ledger) {
    ...
    const resellTicketId = meta.userTicketId;
    if (meta.resellListingId && resellTicketId) {
        const [resellRows] = await pool.query(`SELECT COUNT(*) AS cnt FROM user_tickets
       WHERE id = :ticketId AND user_id = :userId AND order_id = :orderId`, { ticketId: resellTicketId, userId: ledger.user_id, orderId: ledger.order_id });
        return Number(resellRows[0]?.cnt ?? 0);
    }
    return 0;
}
/** Ledgers that may have been charged at PayChangu but never received tickets. */
export async function listLedgersNeedingReconciliation(limit = 50) {
    const sinceDate = RECONCILIATION_SINCE;
    const [rows] = await pool.query(`SELECT pl.*
     ...
     LIMIT :limit`, { limit, sinceDate });
    const candidates = rows;
    const needsRecovery = [];
    for (const ledger of candidates) {
        const ticketCount = await countFulfillmentTickets(ledger);
        if (ticketCount === 0)
            needsRecovery.push(ledger);
    }
    return needsRecovery;
}
export async function attemptPaymentRecovery(ledgerId, options) {
    ...
    if (!options?.skipPaychanguVerify) {
        const verify = await verifyMobileMoneyCharge(ledger.paychangu_charge_id, new Date(ledger.created_at));
        await pool.query(`UPDATE payment_ledger SET provider_status = :providerStatus, last_polled_at = NOW()
       WHERE id = :ledgerId`, { ledgerId, providerStatus: verify.providerStatus });
        if (!verify.success) {
            return {
