...
import { initiateMobileMoneyRefund, refundCardCharge, } from "./paychangu.service.js";
function refundChargeId(refundId) {
    return `TM-REF-${refundId.replace(/-/g, "").slice(0, 24)}`;
}
/**
 * Send refund funds via the buyer's original payment method (Airtel, TNM, or card).
 */
export async function executeCustomerRefundPayment(input) {
    const [rows] = await pool.query(`SELECT o.payment_method, o.payment_phone, pl.paychangu_charge_id
     FROM orders o
     LEFT JOIN payment_ledger pl ON pl.order_id = o.id AND pl.status = 'completed'
     WHERE o.id = :orderId
     LIMIT 1`, { orderId: input.orderId });
    const row = rows[0];
    if (!row)
    ...
    if (env.paychangu.mock) {
        console.log("[refund] mock payout", method, input.refundAmount, originalChargeId ?? row.payment_phone);
        return;
    }
    ...
    if (method === "card") {
        if (!originalChargeId)
            throw new Error("Original card charge not found for refund");
...
export function refundPaymentMethodLabel(method) {
    if (method === "airtel")
        return "Airtel Money";
    if (method === "tnm")
        return "TNM Mpamba";
