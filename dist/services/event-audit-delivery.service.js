...
import { buildEventAuditSnapshot, hasAutoAuditReport, recordAuditReport, recordAuditDeliverySkip, tryResolveOrganizerContact, } from "./event-audit.service.js";
...
export async function processEventAuditReports() {
    let rows = [];
    try {
        const [result] = await pool.query(`SELECT l.id AS listingId, l.title, l.organizer_id AS organizerId,
    ...
    for (const row of rows) {
        ...
        const organizerId = row.organizerId;
        ...
        try {
            ...
            if (!contact) {
                ...
                skipped++;
                console.warn("[event-audit] Skipped (no organizer email):", listingId, row.title);
                continue;
            }
