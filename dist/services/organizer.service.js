...
export async function getBuyers(organizerId) {
    const [rows] = await pool.query(`SELECT ut.id AS purchase_id, o.contact_name AS name, o.contact_email AS email,
     ...
     WHERE l.organizer_id = :organizerId
     ORDER BY ut.purchased_at DESC`, { organizerId });
    return rows.map((r) => ({
...
export async function getOrganizerProfile(userId) {
    const [rows] = await pool.query(`SELECT * FROM organizer_profiles WHERE user_id = :userId`, { userId });
    const row = rows[0];
    if (!row)
        return null;
    return {
...
export async function updateOrganizerProfile(userId, data) {
