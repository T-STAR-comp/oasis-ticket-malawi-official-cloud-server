...
export async function getReferrerFinance(userId) {
    const [earnRows] = await pool.query(`SELECT
     ...
     WHERE re.referrer_user_id = :userId`, { userId });
    const earn = earnRows[0];
    const [payoutRows] = await pool.query(`SELECT
     ...
     FROM referrer_payouts WHERE referrer_user_id = :userId`, { userId });
    ...
    const [refCheck] = await pool.query(`SELECT 1 FROM referrer_profiles WHERE user_id = :userId
     UNION SELECT 1 FROM referral_earnings WHERE referrer_user_id = :userId LIMIT 1`, { userId });
    return {
...
export async function getReferrerEarningsLines(userId, limit = 50) {
