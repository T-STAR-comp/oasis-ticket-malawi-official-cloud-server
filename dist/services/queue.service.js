...
async function expireStaleQueueEntries(listingId) {
    await pool.query(`UPDATE checkout_queue
     SET status = 'expired'
     WHERE listing_id = :listingId
       AND status = 'ready'
       AND ready_expires_at IS NOT NULL
       AND ready_expires_at < NOW()`, { listingId });
}
async function promoteWaitingEntries(listingId, kind, ticketCapacity) {
    await expireStaleQueueEntries(listingId);
    const { remaining } = await getCapacityInfo(listingId, kind, ticketCapacity);
    const slots = Math.min(MAX_READY_SLOTS, remaining != null ? Math.max(1, remaining) : MAX_READY_SLOTS);
    const [readyRows] = await pool.query(`SELECT COUNT(*) AS cnt FROM checkout_queue
     WHERE listing_id = :listingId AND status = 'ready'`, { listingId });
    const readyCount = Number(readyRows[0]?.cnt ?? 0);
    const toPromote = Math.max(0, slots - readyCount);
    if (toPromote <= 0)
        return;
    const [waiting] = await pool.query(`SELECT id FROM checkout_queue
     WHERE listing_id = :listingId AND status = 'waiting'
     ORDER BY created_at ASC
     LIMIT ${toPromote}`, { listingId });
    for (const row of waiting) {
        await pool.query(`UPDATE checkout_queue
       SET status = 'ready', ready_at = NOW(),
           ready_expires_at = DATE_ADD(NOW(), INTERVAL ${READY_WINDOW_SEC} SECOND)
       WHERE id = :id AND status = 'waiting'`, { id: row.id });
    }
}
async function getQueuePosition(queueId) {
    const [rows] = await pool.query(`SELECT cq.id, cq.listing_id, cq.status, cq.created_at
     FROM checkout_queue cq WHERE cq.id = :queueId`, { queueId });
    const entry = rows[0];
    if (!entry)
        return 0;
    if (entry.status === "ready")
        return 1;
    const [posRows] = await pool.query(`SELECT COUNT(*) AS ahead FROM checkout_queue
     WHERE listing_id = :listingId AND status = 'waiting'
       AND created_at < :createdAt`, { listingId: entry.listing_id, createdAt: entry.created_at });
    return Number(posRows[0]?.ahead ?? 0) + 1;
}
async function countWaiting(listingId) {
    const [rows] = await pool.query(`SELECT COUNT(*) AS cnt FROM checkout_queue
...
export async function assertQueueCheckoutAllowed(listingId, userId, queueId, kind, ticketCapacity) {
    ...
    if (entry.status !== "ready") {
        throw new Error("Your checkout turn is not ready yet. Stay on the queue page until you are called.");
    }
    if (entry.ready_expires_at &&
        new Date(entry.ready_expires_at).getTime() < Date.now()) {
        await pool.query(`UPDATE checkout_queue SET status = 'expired' WHERE id = :id`, { id: queueId });
        throw new Error("Your checkout window expired. Please rejoin the queue.");
    }
}
export async function completeQueueEntry(queueId) {
    if (!queueId)
        return;
    await pool.query(`UPDATE checkout_queue
...
export async function pollQueueStatus(queueId, userId) {
    ...
    await promoteWaitingEntries(entry.listing_id, entry.kind, entry.ticket_capacity != null ? Number(entry.ticket_capacity) : null);
    const [refreshed] = await pool.query(`SELECT * FROM checkout_queue WHERE id = :queueId`, { queueId });
    const current = refreshed[0];
    if (!current)
        throw new Error("Queue entry not found");
    const position = current.status === "ready" ? 1 : await getQueuePosition(queueId);
    return {
        queueId,
