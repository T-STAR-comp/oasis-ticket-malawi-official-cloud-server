...
export async function getUserTicketDetail(userId, ticketId) {
    const [rows] = await pool.query(`SELECT * FROM user_tickets WHERE id = :ticketId AND user_id = :userId`, { ticketId, userId });
    const purchase = rows[0];
    if (!purchase)
        return null;
    const refundPending = await ticketHasPendingRefund(purchase.id);
    const listing = await getListingById(purchase.listing_id, true);
