...
export function textContainsMalawiPlace(text) {
...
export function extractMalawiPlace(text) {
    const hay = normalizeForMatch(text);
    if (!hay)
        return null;
    for (const city of MALAWI_CITIES) {
        if (hay.includes(normalizeForMatch(city)))
            return city;
    }
    return null;
}
export function assertListingLocation(kind, status, location, routeFrom, routeTo) {
    ...
    if (kind === "event") {
        ...
        if (!textContainsMalawiPlace(loc)) {
            throw new Error(`Location must include a Malawian city or town (${MALAWI_CITIES.slice(0, 5).join(", ")}, …).`);
        }
        return;
    }
