...
function formatPrice(mwk) {
    if (mwk == null || !Number.isFinite(mwk))
        return "";
    return `MWK ${mwk.toLocaleString("en-MW")}`;
}
function listingToCard(raw) {
...
function renderListingItem(card) {
    const href = `/ticket/${encodeURIComponent(card.id)}`;
    const meta = card.kind === "travel" && card.route
...
function buildJsonLd(events, travel) {
...
export async function buildSeoSnapshot(page) {
...
function filterSnapshotForPage(snapshot, page) {
    if (page === "home")
        return snapshot;
    const title = page === "events"
        ? "Events in Malawi — Ticket Malawi"
        : "Intercity bus travel — Ticket Malawi";
    const intro = page === "events"
