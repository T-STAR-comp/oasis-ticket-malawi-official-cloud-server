...
export function parseEventStartsAt(eventStartsOn, timeLabel) {
    if (!eventStartsOn)
        return null;
    const datePart = eventStartsOn instanceof Date
...
export function assertVirtualMeetingUrl(url) {
...
export function assertVirtualEventPurchasable(input) {
    if (!isVirtualEventPurchasable(input)) {
        throw new Error("This virtual event has already started and is no longer available for purchase.");
    }
}
