...
adminRouter.patch("/finance/settings/service-fee-bearer", async (req, res, next) => {
    try {
        ...
        if (err instanceof z.ZodError)
            return fail(res, "Invalid bearer value", 400);
        next(err);
    }
});
adminRouter.patch("/finance/settings/dynamic-service-fee", async (req, res, next) => {
    try {
        ...
        if (err instanceof z.ZodError)
            return fail(res, "Invalid dynamic fee settings", 400);
        if (err instanceof Error)
            return fail(res, err.message, 400);
        next(err);
    }
...
adminRouter.get("/finance/custom-service-fees/lookup", async (req, res, next) => {
    try {
        ...
        if (!organizer)
            return fail(res, "No organizer found with that email", 404);
...
adminRouter.put("/finance/custom-service-fees", async (req, res, next) => {
    try {
        ...
        if (!organizer)
            return fail(res, "No organizer found with that email", 404);
        await platformSettingsService.setOrganizerCustomServiceFee(organizer.organizerUserId, body.feePercent, admin.id, body.notes);
        return ok(res, await platformSettingsService.getFinanceSettingsSnapshot());
    }
    catch (err) {
        if (err instanceof z.ZodError)
            return fail(res, "Invalid custom fee payload", 400);
        if (err instanceof Error)
            return fail(res, err.message, 400);
        next(err);
    }
...
adminRouter.post("/information/send-email", async (req, res, next) => {
    try {
        ...
        if (err instanceof z.ZodError)
            return fail(res, "Invalid email payload", 400);
        if (err instanceof Error)
            return fail(res, err.message, 400);
        next(err);
    }
...
adminRouter.get("/esports/events/:id", async (req, res, next) => {
    try {
        ...
        if (!event)
            return fail(res, "E-Sports event not found", 404);
        return ok(res, event);
    }
    catch (err) {
        next(err);
...
adminRouter.post("/esports/events", async (req, res, next) => {
    try {
        ...
        if (err instanceof z.ZodError)
            return fail(res, "Invalid event data", 400);
        if (err instanceof Error)
            return fail(res, err.message, 400);
        next(err);
    }
...
adminRouter.patch("/esports/events/:id/match-access", async (req, res, next) => {
    try {
        ...
        if (err instanceof z.ZodError)
            return fail(res, "Invalid match access data", 400);
        if (err instanceof Error)
            return fail(res, err.message, 400);
        next(err);
    }
...
adminRouter.post("/esports/events/:id/settle-winner", async (req, res, next) => {
    try {
        ...
        if (err instanceof z.ZodError)
            return fail(res, "Invalid winner settlement data", 400);
        if (err instanceof Error)
            return fail(res, err.message, 400);
        next(err);
    }
