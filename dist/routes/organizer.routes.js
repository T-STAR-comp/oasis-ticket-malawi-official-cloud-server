...
import { assertUploadRateLimit, processAndStoreListingImage, } from "../services/image-upload.service.js";
import * as referralService from "../services/referral.service.js";
import { fail, ok } from "../utils/http.js";
function forwardListingMutationError(err, res, next) {
...
organizerRouter.post("/appeal", async (req, res, next) => {
    try {
        ...
        const body = z
            ...
            .parse(req.body);
        const result = await moderationService.submitAppeal(user.id, body.reason, body.appealType ?? "suspension");
...
organizerRouter.post("/uploads/listing-image", (req, res, next) => {
    listingImageUpload(req, res, async (err) => {
        ...
        try {
            ...
            const result = await processAndStoreListingImage(user.id, req.file.buffer, req.file.mimetype);
            return ok(res, result, 201);
        }
        catch (uploadErr) {
...
organizerRouter.patch("/listings/:id/status", async (req, res, next) => {
    try {
        ...
        const updated = await listingsService.updateListingStatus(req.params.id, user.id, body.status);
        if (!updated)
            return fail(res, "Listing not found", 404);
...
organizerRouter.patch("/listings/:id/seats/:seatNumber", async (req, res, next) => {
    try {
        ...
        const { status } = z
            ...
            .parse(req.body);
        const listing = await listingsService.updateSeatStatus(req.params.id, user.id, seatNumber, status);
        return ok(res, listing);
    }
    catch (err) {
        if (err instanceof Error) {
...
organizerRouter.post("/payouts/confirm", async (req, res, next) => {
    try {
        ...
        const body = z
            ...
            .parse(req.body);
        const result = await payoutService.confirmPayoutVerification(user.id, body.verificationId, body.code);
        return ok(res, result);
    }
    catch (err) {
        if (err instanceof Error && err.message.length < 240) {
...
organizerRouter.get("/verify/users", async (req, res, next) => {
    try {
        ...
        if (err instanceof z.ZodError)
            return fail(res, "Enter at least 3 characters to search", 400);
        next(err);
    }
});
...
organizerRouter.delete("/verify/assignments/:assignmentId", async (req, res, next) => {
    try {
        const user = req.user;
        const result = await verificationService.revokeVerifierAssignment(user.id, req.params.assignmentId);
        return ok(res, result);
    }
    catch (err) {
        if (err instanceof Error && err.message.length < 240) {
...
organizerRouter.post("/verify/assignments", async (req, res, next) => {
    try {
        ...
        const body = z
            ...
            .parse(req.body);
        const assignment = await verificationService.assignListingVerifier(user.id, body.listingId, body.verifierEmail);
...
organizerRouter.post("/referrals", async (req, res, next) => {
    try {
        ...
        const origin = `${req.protocol}://${req.get("host")}`;
        return ok(res, { ...referral, link: referralService.buildReferralLink(referral.listingId, referral.code, origin) }, 201);
    }
    catch (err) {
        if (err instanceof z.ZodError)
            return fail(res, "Invalid referral data", 400);
        if (err instanceof Error)
            return fail(res, err.message, 400);
        next(err);
    }
...
organizerRouter.patch("/referrals/:id/status", async (req, res, next) => {
    try {
        ...
        const referral = await referralService.setReferralStatus(user.id, String(req.params.id), body.status);
