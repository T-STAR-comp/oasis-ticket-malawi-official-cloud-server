...
dashboardRouter.get("/tickets/:id/share/recipient", async (req, res, next) => {
    try {
        ...
        if (detail.purchase.refundPending) {
            return fail(res, "This ticket has a refund in progress. Sharing is disabled until the refund completes.", 400);
        }
...
dashboardRouter.post("/tickets/:id/share", async (req, res, next) => {
    try {
        ...
        if (err instanceof Error && err.message.length < 240) {
            const status = err.message.includes("not found") || err.message.includes("no longer active")
                ? 404
                : 400;
            return fail(res, err.message, status);
        }
...
dashboardRouter.post("/resell/payouts/confirm", async (req, res, next) => {
    try {
        ...
        const body = z
            ...
            .parse(req.body);
        const result = await resellerPayoutService.confirmResellerPayoutVerification(user.id, body.verificationId, body.code);
